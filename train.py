...
def train():
    ...
    gpu_count = max(torch.cuda.device_count() if args.cuda else 1, 1)
    per_epoch_size = len(train_dataset) // (batch_size * gpu_count)
    ...
    device = torch.device('cuda' if args.cuda and torch.cuda.is_available() else 'cpu')
    net_enh.load_state_dict(torch.load(args.save_folder + 'decomp.pth', map_location=device))
    ...
    device = torch.device("cuda" if args.cuda and torch.cuda.is_available() else "cpu")
    criterion = MultiBoxLoss(cfg, use_gpu=args.cuda, device=device)

    criterion_enhance = EnhanceLoss(device=device)
    if local_rank == 0:
    ...
    for epoch in range(start_epoch, cfg.EPOCHES):
        ...
        for batch_idx, (images, targets, _) in enumerate(train_loader):
            ...
            targetss = [Variable(ann, requires_grad=False)
                        for ann in targets]
            img_dark = torch.empty(size=(images.shape[0], images.shape[1], images.shape[2], images.shape[3]))
