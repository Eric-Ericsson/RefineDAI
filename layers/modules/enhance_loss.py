from __future__ import division
from __future__ import absolute_import
from __future__ import print_function

import torch
import torch.nn as nn
import torch.nn.functional as F
from torchmetrics.functional import structural_similarity_index_measure as ssim
from data.config import cfg


def gradient(input_tensor, direction, device):
    channels = input_tensor.shape[1]

    if direction == "x":
        kernel = torch.FloatTensor([[0, 0], [-1, 1]]).view(1, 1, 2, 2).to(device)
    elif direction == "y":
        kernel = torch.FloatTensor([[0, -1], [0, 1]]).view(1, 1, 2, 2).to(device)

    kernel = kernel.repeat(channels, 1, 1, 1)
    return F.conv2d(input_tensor, kernel, padding=1, groups=channels)

def ave_gradient(input_tensor, direction, device):
    return torch.abs(gradient(input_tensor, direction, device))

def smooth(input_I, input_R, device):
    return torch.mean(
        gradient(input_I, "x", device) * torch.exp(-10 * ave_gradient(input_R, "x", device)) +
        gradient(input_I, "y", device) * torch.exp(-10 * ave_gradient(input_R, "y", device))
    )

class EnhanceLoss(nn.Module):
    def __init__(self, device='cpu'):
        super(EnhanceLoss, self).__init__()
        self.device = device

    def forward(self, preds, img, img_dark):
        R_dark, R_light, R_dark_2, R_light_2, I_dark, I_light = preds

        losses_equal_R = (F.mse_loss(R_dark, R_light.detach())) * cfg.WEIGHT.EQUAL_R
        losses_recon_low = F.mse_loss(R_dark * I_dark, img_dark) * 1.+ (1. - ssim(R_dark * I_dark, img_dark))
        losses_recon_high = F.mse_loss(R_light * I_light, img) * 1.+ (1. - ssim(R_light * I_light, img))

        losses_smooth_low = smooth(I_dark, R_dark, self.device) * cfg.WEIGHT.SMOOTH

        losses_smooth_high = smooth(I_light, R_light, self.device) * cfg.WEIGHT.SMOOTH
        # Redecomposition cohering loss
        losses_rc = (F.mse_loss(R_dark_2, R_dark.detach()) + F.mse_loss(R_light_2, R_light.detach())) * cfg.WEIGHT.RC

        enhance_loss = losses_equal_R + losses_recon_low + losses_recon_high + losses_smooth_low \
                       + losses_smooth_high + losses_rc

        return enhance_loss